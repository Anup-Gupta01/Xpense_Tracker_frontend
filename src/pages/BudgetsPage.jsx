...
import { budgetsMock } from '../data/expensesMockData'
...
const EMPTY_FORM = { category: 'Food & Dining', limit: '', period: 'Jun 2026' }
...
export default function BudgetsPage() {
  const [budgets, setBudgets]   = useState(budgetsMock)
  ...
  function handleSave() {
    ...
    const catMeta = CATEGORY_OPTIONS.find(c => c.label === form.category) || CATEGORY_OPTIONS[0]
    const entry = {
      id:       editId ?? `b${++nextBudgetId}`,
      category: form.category,
      icon:     catMeta.icon,
      color:    catMeta.color,
      colorBg:  catMeta.colorBg,
      limit:    parseFloat(form.limit),
      spent:    editId ? budgets.find(b => b.id === editId)?.spent ?? 0 : 0,
      transactions: editId ? budgets.find(b => b.id === editId)?.transactions ?? 0 : 0,
      period:   form.period || 'Jun 2026',
    }
    ...
    >
      ...
      <div className="bud-summary-row">
        <div className="bud-summary-card card">
          <div className="bud-summary-icon bud-icon-teal">
            <DollarSign size={18} />
          </div>
          <div>
        ...
        <div className="bud-summary-card card">
          <div className="bud-summary-icon bud-icon-indigo">
            <TrendingUp size={18} />
          </div>
          <div>
            ...
            <div key={budget.id} className={`bud-card card ${isOver ? 'bud-card-over' : ''}`} id={`budget-${budget.id}`}>
              ...
              <div className="bud-card-header">
                ...
                <div className="bud-card-actions">
                  {isOver && (
                    <span className="bud-warn-badge">
                      <AlertTriangle size={11} />
                      Over Budget
                    </span>
                  )}
                  {isWarn && !isOver && (
                    <span className="bud-warn-badge bud-warn-yellow">
                      <AlertTriangle size={11} />
                      Near Limit
                    </span>
                  )}
              ...
              <div className="bud-progress-wrap" role="progressbar" aria-valuenow={p} aria-valuemin={0} aria-valuemax={100}>
                <div
                  ...
                  style={{
                    width: `${p}%`,
                    background: isOver ? 'var(--error)' : isWarn ? '#f59e0b' : budget.color,
                  }}
                />
              </div>
        ...
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <div className="modal-box" id="budget-modal">
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-teal" onClick={handleSave} id="budget-save-btn">
                <Check size={14} />
                {modal === 'add' ? 'Create Budget' : 'Save Changes'}
              </button>
            </div>
          </div>
        </div>
